from . import models, utils, dataparser, estantevirtual_dicts
# import utils
import re
import bs4
import httpx
...
class Fetcher:
    ...
    def get_count(self) -> None:
        if self._search_initiated() and self._is_valid_page():
            counter = self.soup.find("p", class_='product-list-header__sort__text', text=re.compile(r".*Exibindo: \d.*"))
            if new_count := re.search(r'.*?Exibindo\:\D*\d+.*?\D([\d\.]+)\s*resultados[\s\n]*$', counter.text):
    ...
    def check_count(self) -> None:
        ...
        if self.website_count:
            if not new_count == self.website_count:
                raise utils.ContentUpdatedError(f'Found different count of books in the same search! \n{new_count=} \n{self.website_count=}')
            else:
    ...
    def fetch_page(self) -> str:
        self.response = self.session.get(
            url=f"/busca",
            params=estantevirtual_dicts.get_request_params(self.query, self.page_number),
    ...
    def fetch_all_pages(self):
        while True and self.saved_count < self.limit:
            ...
            if not self._is_valid_page():
                break
            for item in dataparser.DataParser(self.response.text).get_parsed_data():
                yield item
                self.saved_count += 1
    ...
    def _is_valid_page(self) -> bool:
        return bool(
            not self.soup.find(text=re.compile('.*(Sua pesquisa não retornou nenhum resultado).*')) and \
            not self.soup.find() is None and \
            self._search_initiated()
        )
